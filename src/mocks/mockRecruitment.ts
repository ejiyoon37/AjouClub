// (수정) ApiRecruitmentPost 임포트 구문 삭제
// import type { ApiRecruitmentPost as RecruitmentPost } from "../Hooks/useRecruitments";

// (수정) 타입을 any로 변경 (혹은 ApiRecruitmentPost 타입을 직접 정의해야 하나, 현재 사용되지 않으므로 any로 둠)
export const mockRecruitments: any[] = [
  {
    id: 1,
    clubId: 101, 
    imageUrl: 'src/assets/img/AKO.jpg',
    title: '💙 24-2 SWeat 신입 부원 모집 💙',
    recruitmentStatus: 'regular',
    dDay: 10,
    viewCount: 120,
    saveCount: 10,
    createdAt: '2025-09-10',
  },
  {
    id: 2,
    clubId: 102, 
    imageUrl: 'src/assets/img/R_image.png',
    title: '[마감임박] 나비야 밴드부에서 드러머 구합니다!',
    recruitmentStatus: 'd-day',
    dDay: 2,
    viewCount: 85,
    saveCount: 20,
    createdAt: '2025-09-11',
  },
  {
    id: 3,
    clubId: 103, 
    imageUrl: 'src/assets/img/AKO.jpg',
    title: '기독교 동아리 [JOY] 24-2학기 모집',
    recruitmentStatus: 'regular',
    dDay: 0,
    viewCount: 40,
    saveCount: 5,
    createdAt: '2025-09-09',
  },
  {
    id: 4,
    clubId: 104, 
    imageUrl: 'src/assets/img/R_image.png',
    title: 'C.O.D.E. - 개발 동아리 2기 모집',
    recruitmentStatus: 'end',
    dDay: 0,
    viewCount: 300,
    saveCount: 55,
    createdAt: '2025-08-30',
  },
  {
    id: 5,
    clubId: 105, 
    imageUrl: 'src/assets/img/AKO.jpg',
    title: 'SCAU에서 애니메이션 덕후 찾습니다!',
    recruitmentStatus: 'd-day',
    dDay: 1,
    viewCount: 75,
    saveCount: 12,
    createdAt: '2025-09-13',
  },
  {
    id: 6,
    clubId: 106, 
    imageUrl: 'src/assets/img/R_image.png',
    title: '수학 문제 풀이 동아리 [정답은 42]',
    recruitmentStatus: 'regular',
    dDay: 12,
    viewCount: 60,
    saveCount: 8,
    createdAt: '2025-09-07',
  },
  {
    id: 7,
    clubId: 107, 
    imageUrl: 'src/assets/img/AKO.jpg',
    title: '🧁 베이킹 소모임 [오븐 속 이야기] 신입 모집',
    recruitmentStatus: 'regular',
    dDay: 5,
    viewCount: 90,
    saveCount: 14,
    createdAt: '2025-09-12',
  },
  {
    id: 8,
    clubId: 108, 
    imageUrl: 'src/assets/img/R_image.png',
    title: '[시네마 클럽] 단편영화 제작팀원 모집',
    recruitmentStatus: 'end',
    dDay: 0,
    viewCount: 110,
    saveCount: 25,
    createdAt: '2025-08-25',
  },
  {
    id: 9,
    clubId: 109, 
    imageUrl: 'src/assets/img/AKO.jpg',
    title: '🌿 환경보호 동아리 [GreenUp]',
    recruitmentStatus: 'd-day',
    dDay: 3,
    viewCount: 45,
    saveCount: 6,
    createdAt: '2025-09-14',
  },
  {
    id: 10,
    clubId: 110, 
    imageUrl: 'src/assets/img/R_image.png',
    title: '🔥 운동 동아리 [IRONCLUB] 24-2 기수 모집',
    recruitmentStatus: 'regular',
    dDay: 20,
    viewCount: 130,
    saveCount: 35,
    createdAt: '2025-09-05',
  },
  {
    id: 11,
    clubId: 111, 
    imageUrl: 'src/assets/img/AKO.jpg',
    title: '🎮 게임 개발자 모임 [UnityVerse]',
    recruitmentStatus: 'end',
    dDay: 0,
    viewCount: 70,
    saveCount: 18,
    createdAt: '2025-08-20',
  },
  {
    id: 12,
    clubId: 112, 
    imageUrl: 'src/assets/img/R_image.png',
    title: '✏️ 창작 글쓰기 동아리 [흑백논리]',
    recruitmentStatus: 'regular',
    dDay: 7,
    viewCount: 95,
    saveCount: 16,
    createdAt: '2025-09-08',
  },
];